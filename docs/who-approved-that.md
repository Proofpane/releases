...
| A few times a month | Activate a policy version · change a model or provider · register a governed device · export evidence externally · elevate a role | R2 Per-action signature | These change the rules for everything downstream. Each one deserves a signature bound to that exact action. Five seconds on a phone or laptop — and that is the honest limit of a phone: it signs, but it classifies at the **lower** of R2’s two tiers. Only an authenticator whose attestation verifies *and* chains to a FIDO-certified root is labelled hardware-backed, which in practice means a security key. Measured, not assumed: a laptop Secure Enclave credential came back l2_standard ; a FIDO2 key came back l2_high . |
