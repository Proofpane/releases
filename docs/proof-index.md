> Mirror of <https://proofpane.com/proof/> · synced 2026-08-09 · the canonical, always-current version lives on proofpane.com
...
| Security posture (data flow, encryption, tenant isolation, roadmap) | Plain-text, item-by-item: the current distributed macOS daemon and public Tray v0.3.8 macOS DMG carry Apple signing/notarisation evidence; the desktop CI build remains separately labelled from public distribution; SOC 2, a third-party pen test and Windows Authenticode remain `[roadmap]` — macOS signing is done, it is the Windows half that is outstanding: [Trust Center](https://proofpane.com/trust/) | PUBLISHED |
